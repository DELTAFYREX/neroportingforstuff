...
class Gun extends EventEmitter {
    constructor(body, info) {
        ...
        this.body = body;
        this.ignorecanshootchecks = false;
        ...
        if (info.PROPERTIES != null) {
            ...
            this.ignorecanshootchecks = info.PROPERTIES.IGNORES_CANSHOOT_CHECKS ?? false;
    ...
    createBullet(spawnX, spawnY) {
        ...
        // Find velocity
        let velocityMagnitude = this.negativeRecoil * this.shootSettings.speed * this.bulletSkills.spd * (shudder + 1) * Config.runSpeed * 1.3,
    ...
    calculateBulletStats() {
        ...
        if ((!this.ignorecanshootchecks && !this.canShoot) || this.independentChildren) return;
        ...
        switch (this.statCalculator) {
            ...
            case "fixedReload":
                ...
                break;
            case "lancer":
                out.RANGE = 0.02 * this.bulletSkills.spd;
                break;
            case "necro":
...
class Prop {
    ...
    define(def, keepteam = false) {
        ...
        if (keepteam) {
          ...
          if (set.VARIABLES != null) this.variables = set.VARIABLES ? JSON.parse(JSON.stringify(set.VARIABLES)) : {};
        }
    }
    camera() {
        return {
    ...
    move() {
        ...
        switch (this.motionType) {
            ...
            case "fuckingnuclearbomb":
                this.SIZE += this.motionTypeArgs.growSpeed ?? 10;
                break;
            case "trappershockwave":
                this.SIZE += this.motionTypeArgs.growSpeed ?? 20;
                break;
            case "shrink":
                this.SIZE -= this.motionTypeArgs.growSpeed ?? 1;
                break;
            case "solarioarena":
                ...
                this.SIZE += this.motionTypeArgs.growSpeed ?? 2;
                break;
            case "glide":
                this.maxSpeed = this.topSpeed;
            ...
            for (let i = 0; i < killers.length; i++) {
                ...
                switch (this.type) {
                    case "tank":
                        ...
                        switch(instance.killCount.solo) {
                          ...
                          case 25:
                              sockets.broadcast(instance.name + " is on a kill streak of 25!");
                          break;
                            
                          case 50:
                              sockets.broadcast(instance.name + " is on a kill streak of 50!");
                          break;
                            
                          case 100:
                              sockets.broadcast(instance.name + " has become the Harbinger of Death");
                          break;
